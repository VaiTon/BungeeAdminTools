...
public class BanCommand extends CommandHandler {
	...
	public static void handleBanCommand(final BATCommand command, final boolean global, final boolean ipBan,
		...
		if (global) {
			...
			if (args.length == 1) {
				checkArgument(sender instanceof ProxiedPlayer, I18n.getFormatted("specifyServer"));
				server = ((ProxiedPlayer) sender).getServer().getInfo().getName();
			} else {
				checkArgument(Utils.isServer(args[1]), I18n.getFormatted("invalidServer"));
				server = args[1];
		...
		if (!Utils.validIP(target) && player == null && pUUID == null) {
			...
			if (ipBan) {
				checkArgument(!"0.0.0.0".equals(ip), I18n.getFormatted("ipUnknownPlayer"));
			}
			// If ip = 0.0.0.0, it means the player never connects
				...
				if ("0.0.0.0".equals(ip) && !confirmedCmd) {
					command.mustConfirmCommand(sender, command.getName() + " " + Joiner.on(' ').join(args),
							I18n.getFormatted("operationUnknownPlayer", new String[] { target }));
					return;
				}
		...
		checkArgument(!PermissionManager.isExemptFrom(Action.BAN, target), I18n.getFormatted("isExempt"));

		checkArgument(!ban.isBan((ip == null) ? target : ip, server), I18n.getFormatted("alreadyBan"));
	...
	public static void handleTempBanCommand(final BATCommand command, final boolean global, final boolean ipBan,
		...
		if (global) {
			...
			if (args.length == 2) {
				...
				checkArgument(Utils.isServer(args[2]), I18n.getFormatted("invalidServer"));
				server = args[2];
		...
		checkArgument(!PermissionManager.isExemptFrom(Action.BAN, target), I18n.getFormatted("isExempt"));
		checkArgument(!ban.isBan(target, server), I18n.getFormatted("alreadyBan"));
	...
	public static void handlePardonCommand(final BATCommand command, final boolean global, final boolean ipUnban,
		...
		checkArgument(
				...
				(IModule.ANY_SERVER.equals(server) ? I18n.getFormatted("notBannedAny", formatArgs) : ((ipUnban) ? I18n.getFormatted("notBannedIP",
						formatArgs) : I18n.getFormatted("notBanned", formatArgs))));
  ...
  public static class BanListCmd extends BATCommand {
    ...
    public void onCommand(final CommandSender sender, final String[] args,
      ...
      msg.append(I18n.getFormatted("banListHeader").replace("{page}", String.valueOf(page)));
      msg.append("\n");
      ...
      for(BanEntry ban : bans){
          ...
          if(ban.isActive()){
            ...
            msg.append(I18n.getFormatted("activeBanListRow",
            ...
            msg.append(I18n.getFormatted("archiveBanListRow", new String[] { ban.getEntity(), begin, ban.getServer(), ban.getReason(),
